...
public class FollowTask extends BukkitRunnable {
    ...
    public FollowTask(Arrow arrow, LivingEntity target, Main instance) {
        this.arrow = arrow;
        this.target = target;
    ...
    public void run() {
        ...
        if ((this.arrow.isOnGround()) || (this.arrow.isDead()) || (this.target.isDead()) || ((Player)this.target).isBlocking()) {
            cancel();
            return;
        }

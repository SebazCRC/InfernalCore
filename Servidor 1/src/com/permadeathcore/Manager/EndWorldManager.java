...
import com.permadeathcore.End.Util.Cuboid;
...
public class EndWorldManager implements Listener {
    ...
    @EventHandler
    public void BedrockPillar(ChunkLoadEvent e) {

        if (main.endWorld == null) return;
        if (main.getDays() < 30) return;
            ...
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    for (int y = 0; y < 256; y++) {
                        ...
                        if (c.getWorld().getBlockAt(X + x, y, Z + z).getType() == Material.OBSIDIAN) {
                            if (c.getWorld().getBlockAt(X + x, y, Z + z).getRelative(BlockFace.DOWN).getType() != Material.AIR) {
                                c.getWorld().getBlockAt(X + x, y, Z + z).setType(Material.BEDROCK);
                            }
                        }
                    }
                }
